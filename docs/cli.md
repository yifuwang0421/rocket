# rocket — CLI Reference

Terminal client for the Rocket server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running Rocket headless server with URL and token

## Installation

From the Rocket repository root:
...
# Option B: Link globally (adds rocket to PATH)
...
|------|---------|---------|-------------|
| `--url <ws[s]://...>` | `ROCKET_SERVER_URL` | — | Server WebSocket URL |
| `--token <secret>` | `ROCKET_SERVER_TOKEN` | — | Authentication token |
| `--workspace <id>` | — | auto-detect | Workspace ID |
| `--timeout <ms>` | — | `10000` | Request timeout |
| `--tls-ca <path>` | `ROCKET_TLS_CA` | — | Custom CA cert for self-signed TLS |
...
```bash
rocket ping              # Verify connectivity (clientId + latency)
rocket health            # Check credential store health
rocket versions          # Show server runtime versions
```

### Resource Listing

```bash
rocket workspaces        # List all workspaces
rocket sessions          # List sessions in workspace
rocket connections       # List LLM connections
rocket sources           # List configured sources
```

### Session Operations

```bash
rocket session create [--name <n>] [--mode <m>]  # Create session
rocket session messages <id>                       # Print message history
rocket session delete <id>                         # Delete session
rocket cancel <id>                                 # Cancel processing
```
...
rocket send <session-id> <message>

# Pipe text from stdin
echo "Summarize this file" | rocket send <session-id>

# Read from stdin explicitly
cat document.txt | rocket send <session-id> --stdin
```
...
rocket invoke <channel> [json-args...]

# Subscribe to push events (Ctrl+C to stop)
rocket listen <channel>
```

Examples:
```bash
rocket invoke system:homeDir
rocket invoke sessions:get '"workspace-123"'
rocket listen session:event
```
...
```bash
rocket run <prompt>
rocket run --workspace-dir ./project --source github "List open PRs"
```
...
rocket run --provider anthropic --base-url https://openrouter.ai/api/v1 --api-key $OR_KEY "Hello"
```

Prompt can also be piped via stdin:
```bash
echo "Summarize this file" | rocket run
cat error.log | rocket run "What's causing these errors?"
```
...
rocket --validate-server --url ws://127.0.0.1:9100 --token <token>
...
```bash
# Get workspace IDs
WORKSPACES=$(rocket --json workspaces | jq -r '.[].id')

# Count sessions per workspace
for ws in $WORKSPACES; do
  COUNT=$(rocket --json --workspace "$ws" sessions | jq length)
  echo "$ws: $COUNT sessions"
done

# Create a session and capture its ID
SESSION_ID=$(rocket --json session create --name "CI Run" | jq -r '.id')

# Send a message and wait for completion
rocket send "$SESSION_ID" "Run the test suite and report results"

# Clean up
rocket session delete "$SESSION_ID"
```
...
rocket --url wss://server.example.com:9100 ping

# Self-signed certificate
rocket --url wss://server.example.com:9100 --tls-ca /path/to/ca.pem ping
```

The `--tls-ca` flag sets `NODE_EXTRA_CA_CERTS` before connecting. You can also set `ROCKET_TLS_CA` in your environment.
...
| `AUTH_FAILED` | Wrong token | Check `ROCKET_SERVER_TOKEN` matches server |
