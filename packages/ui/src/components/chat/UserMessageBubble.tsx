...
import type { StoredAttachment, ContentBadge } from '@rocket/core'
import { normalizePath } from '@rocket/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .rocket workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.rocket\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
