#!/usr/bin/env bun
/**
 * @rocket/server — standalone headless Rocket server.
 *
 * Usage:
 *   ROCKET_SERVER_TOKEN=<secret> bun run packages/server/src/index.ts
 *
 * Environment:
 *   ROCKET_SERVER_TOKEN         — required bearer token for client auth
 *   ROCKET_RPC_HOST             — bind address (default: 127.0.0.1)
 *   ROCKET_RPC_PORT             — bind port (default: 9100)
 *   ROCKET_RPC_TLS_CERT         — path to PEM certificate file (enables TLS/wss)
 *   ROCKET_RPC_TLS_KEY          — path to PEM private key file (required with cert)
 *   ROCKET_RPC_TLS_CA           — path to PEM CA chain file (optional)
 *   ROCKET_APP_ROOT             — app root path (default: cwd)
 *   ROCKET_RESOURCES_PATH       — resources path (default: cwd/resources)
 *   ROCKET_IS_PACKAGED          — 'true' for production (default: false)
 *   ROCKET_VERSION              — app version (default: 0.0.0-dev)
 *   ROCKET_DEBUG                — 'true' for debug logging
 *   ROCKET_WEBUI_DIR            — path to built web UI assets (enables web UI on RPC port)
 *   ROCKET_WEBUI_PASSWORD       — optional shorter password for web login (falls back to ROCKET_SERVER_TOKEN)
 *   ROCKET_WEBUI_SECURE_COOKIE  — optional true/false override for the session cookie Secure flag
 *   ROCKET_WEBUI_WS_URL         — optional browser-facing ws:// or wss:// URL returned by /api/config
 *   ROCKET_MESSAGING_WA_WORKER  — absolute path to worker.cjs (default: packages/messaging-whatsapp-worker/dist/worker.cjs)
 *   ROCKET_MESSAGING_NODE_BIN   — Node binary used to spawn the WhatsApp worker (default: node)
 */
...
import { enableDebug } from '@rocket/shared/utils/debug'
import { bootstrapServer, startHealthHttpServer, generateServerToken } from '@rocket/server-core/bootstrap'
import { validateSession, createWebuiHandler, nodeHttpAdapter } from '@rocket/server-core/webui'
import type { WebuiHandler } from '@rocket/server-core/webui'
import { getCredentialManager } from '@rocket/shared/credentials'
import { getWorkspaces } from '@rocket/shared/config'
...
import type { WsRpcTlsOptions } from '@rocket/server-core/transport'
...
import { initModelRefreshService, setFetcherPlatform } from '@rocket/server-core/model-fetchers'
import { setSearchPlatform, setImageProcessor } from '@rocket/server-core/services'
import type { HandlerDeps } from '@rocket/server-core/handlers'

process.env.ROCKET_IS_PACKAGED ??= 'false'
...
if (process.env.ROCKET_DEBUG === 'true' || process.env.ROCKET_DEBUG === '1') {
  enableDebug()
}
...
// In packaged mode, use ROCKET_BUNDLED_ASSETS_ROOT env or cwd.
const bundledAssetsRoot = process.env.ROCKET_BUNDLED_ASSETS_ROOT
  ?? join(import.meta.dir, '..', '..', '..', '..')

// TLS configuration — when cert + key paths are provided, server listens on wss://
let tls: WsRpcTlsOptions | undefined
const tlsCertPath = process.env.ROCKET_RPC_TLS_CERT
const tlsKeyPath = process.env.ROCKET_RPC_TLS_KEY
if (tlsCertPath || tlsKeyPath) {
  if (!tlsCertPath || !tlsKeyPath) {
    console.error('TLS requires both ROCKET_RPC_TLS_CERT and ROCKET_RPC_TLS_KEY.')
    process.exit(1)
  }
  tls = {
    cert: readFileSync(tlsCertPath),
    key: readFileSync(tlsKeyPath),
    ...(process.env.ROCKET_RPC_TLS_CA ? { ca: readFileSync(process.env.ROCKET_RPC_TLS_CA) } : {}),
  }
}

// Web UI configuration
const webuiDir = process.env.ROCKET_WEBUI_DIR || undefined
const webuiEnabled = webuiDir && existsSync(webuiDir)
const webuiSecureCookies = parseOptionalBooleanEnv('ROCKET_WEBUI_SECURE_COOKIE', process.env.ROCKET_WEBUI_SECURE_COOKIE)
const webuiWsUrl = parseOptionalWebSocketUrl('ROCKET_WEBUI_WS_URL', process.env.ROCKET_WEBUI_WS_URL)
const serverToken = process.env.ROCKET_SERVER_TOKEN
...
if (webuiEnabled && serverToken) {
  const rpcPort = parseInt(process.env.ROCKET_RPC_PORT ?? '9100', 10)
  const rpcProtocol = tls ? 'wss' as const : 'ws' as const

  webuiHandler = createWebuiHandler({
    webuiDir: webuiDir!,
    secret: serverToken,
    password: process.env.ROCKET_WEBUI_PASSWORD || undefined,
...
const waWorkerEntry = process.env.ROCKET_MESSAGING_WA_WORKER
  ?? join(bundledAssetsRoot, 'packages', 'messaging-whatsapp-worker', 'dist', 'worker.cjs')
const waNodeBin = process.env.ROCKET_MESSAGING_NODE_BIN ?? 'node'
...
const instance = await (async () => {
  try {
    return await bootstrapServer<SessionManager, HandlerDeps>({
      bundledAssetsRoot,
      serverVersion: process.env.ROCKET_VERSION ?? packageVersion,
      tls,
...
// ---------------------------------------------------------------------------
// ROCKET_DISABLE_MESSAGING lets a dev/test server share a config dir with a live app
// without both processes fighting over the same Telegram/WhatsApp connections (409s).
const messagingDisabled = process.env.ROCKET_DISABLE_MESSAGING === 'true' || process.env.ROCKET_DISABLE_MESSAGING === '1'
if (messagingHandle !== null && !messagingDisabled) {
  ...
  console.log('[messaging] Disabled via ROCKET_DISABLE_MESSAGING — skipping workspace messaging init')
}

// Wire up the lazy health check now that the session manager is ready
if (webuiHandler) {
  const { getHealthCheck } = await import('@rocket/server-core/handlers/rpc/server')
  ...
  const { getSourceCredentialManager, loadWorkspaceSources } = await import('@rocket/shared/sources')
  const { getWorkspaceByNameOrId } = await import('@rocket/shared/config')
  const { pushTyped } = await import('@rocket/server-core/transport')
  const { RPC_CHANNELS } = await import('@rocket/shared/protocol')
...
// Start HTTP health endpoint if ROCKET_HEALTH_PORT is set
const healthPort = parseInt(process.env.ROCKET_HEALTH_PORT ?? '0', 10)
...
if (webuiHandler) {
  console.log(`ROCKET_WEBUI_URL=${serverProto}://0.0.0.0:${instance.port}`)
}
...
if (!isLocalBind && instance.protocol === 'ws') {
  if (process.argv.includes('--allow-insecure-bind')) {
    console.warn(
      ...
      '   Set ROCKET_RPC_TLS_CERT and ROCKET_RPC_TLS_KEY to enable wss://.\n'
    )
  } else {
    console.error(
      ...
      '   Options:\n' +
      '     1. Set ROCKET_RPC_TLS_CERT and ROCKET_RPC_TLS_KEY to enable wss://\n' +
