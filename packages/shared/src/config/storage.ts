...
import type { StoredAttachment, StoredMessage } from '@rocket/core/types';
...
import type { Workspace, AuthType } from '@rocket/core/types';
...
const FALLBACK_CONFIG_DEFAULTS: ConfigDefaults = {
  version: '1.0',
  description: 'Default configuration values for Rocket',
  defaults: {
...
/**
 * Load config defaults from ~/.rocket/config-defaults.json
...
export function ensureConfigDir(): void {
  ...
  backupConfigFile();
  // Initialize bundled docs (creates ~/.rocket/docs/ with sources.md, agents.md, permissions.md)
  initializeDocs();
...
export function getBrowserToolEnabled(): boolean {
  ...
  try {
    const defaults = loadConfigDefaults();
    return defaults.defaults.browserToolEnabled ?? true;
  } catch {
    // Headless and first-run callers may query the toggle before bundled
    // defaults have been synchronized. Honor the documented default.
    return true;
  }
}
...
export type { StoredAttachment, StoredMessage } from '@rocket/core/types';
...
/**
 * Get the path to the app-level theme override file (~/.rocket/theme.json).
 */
 ...
 * Preset themes are stored at ~/.rocket/themes/
 */
...
/**
 * Returns the path to the tool-icons directory: ~/.rocket/tool-icons/
 */
