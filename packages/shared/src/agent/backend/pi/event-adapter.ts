 ...
 * Rocket's AgentEvent format for UI compatibility.
 *
...
import type { AgentEvent as CraftAgentEvent } from '@rocket/core/types';
import type {
...
/**
 * Maps Pi SDK events to RocketEvents for UI compatibility.
 *
 * Event mapping:
...
export class PiEventAdapter extends BaseEventAdapter {
  ...
  /**
   * Adapt a Pi SDK event to zero or more RocketEvents.
   */
  *adaptEvent(event: PiEvent): Generator<CraftAgentEvent> {
    ...
    // it to a Craft assistant message via `sdkMessageId`. See rockets-oss#782.
