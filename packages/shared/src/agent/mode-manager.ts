...
import { getSessionSafeAllowedToolNames } from '@rocket/session-tools-core';
...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  if (normalizedTarget.includes('/.rocket/workspaces/') && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside .rocket entirely
  if (!normalizedTarget.includes('/.rocket/')) {
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    if (toolName.startsWith('mcp__rockets-docs__')) {
      return { allowed: true };
    }
