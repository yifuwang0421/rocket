 ...
 *
 * Attaches session-scoped management properties to a SessionToolContext using
...
import type { SessionToolContext } from '@rocket/session-tools-core';
 ...
 *
 * Defines lazy getters for the callbacks supplied by SessionManager, including
 * session metadata, background work, sources, tasks, and Workspace research scope.
 *
...
export function attachSessionSelfManagementBindings(
  ...
  Object.defineProperty(context, 'manageResearchScope', {
    get() {
      return getSessionScopedToolCallbacks(sessionId)?.manageResearchScopeFn;
    },
    configurable: true,
    enumerable: true,
  });
