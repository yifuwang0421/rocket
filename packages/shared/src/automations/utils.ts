...
/**
 * Build the base ROCKET_* environment variables shared by both prompt and webhook actions.
...
function buildBaseEventEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  const env: Record<string, string> = {
    ROCKET_EVENT: event,
    ROCKET_EVENT_DATA: JSON.stringify(payload),
  };

  if (payload.sessionId) env.ROCKET_SESSION_ID = payload.sessionId;
  if (payload.sessionName) env.ROCKET_SESSION_NAME = payload.sessionName;
  if (payload.workspaceId) env.ROCKET_WORKSPACE_ID = payload.workspaceId;
  ...
  if (Object.keys(sessionMetadata).length > 0) {
    env.ROCKET_SESSION_METADATA = JSON.stringify(sessionMetadata);
  }

  // Local time for scheduler events
  if (event === 'SchedulerTick') {
    const now = new Date();
    env.ROCKET_LOCAL_TIME = now.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' });
    env.ROCKET_LOCAL_DATE = now.toISOString().split('T')[0]!;
  }

  // Payload fields as ROCKET_ vars (raw — callers apply sanitization if needed)
  for (const [key, value] of Object.entries(payload)) {
    if (PAYLOAD_SKIP_KEYS.has(key)) continue;
    const envKey = `ROCKET_${toSnakeCase(key).toUpperCase()}`;
...
export function buildEnvFromPayload(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  if (payload.sessionName) env.ROCKET_SESSION_NAME = sanitizeForShell(payload.sessionName);
 ...
 * - Only injects ROCKET_WH_* user-defined vars from process.env (webhook secrets)
 * - Includes ROCKET_* system vars derived from the event payload
 *
 * Users set webhook secrets in their shell profile:
 *   export ROCKET_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
 *   export ROCKET_WH_DISCORD_TOKEN="abc123"
 *
 * Then reference them in automations.json:
 *   "url": "${ROCKET_WH_SLACK_URL}"
 *   "headers": { "Authorization": "Bearer ${ROCKET_WH_DISCORD_TOKEN}" }
 */
export function buildWebhookEnv(event: AutomationEvent, payload: BaseEventPayload): Record<string, string> {
  ...
  // User-defined webhook secrets: only ROCKET_WH_* from process.env
  for (const [key, value] of Object.entries(process.env)) {
    if (key.startsWith('ROCKET_WH_') && value !== undefined) {
      env[key] = value;
    }
  }
