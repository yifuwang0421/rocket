 ...
 *
 * NOTE: rockets-docs is now an always-available MCP server configured
 * directly in rocket.ts, not a source. This file is kept for backwards
 ...
 *
 * Currently returns empty array - rockets-docs has been moved to
 * an always-available MCP server in rocket.ts.
 *
...
/**
 * Get the built-in Rocket docs source.
 *
 * @deprecated rockets-docs is now an always-available MCP server
 * configured directly in rocket.ts. This function is kept for
...
export function getDocsSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const placeholderConfig: FolderSourceConfig = {
    ...
    tagline: 'Search Rocket documentation and source setup guides',
    icon: '📚',
 ...
 *
 * Returns false - rockets-docs is now an always-available MCP server,
