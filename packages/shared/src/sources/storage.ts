 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.rocket/workspaces/xxx)
 ...
 * (like rockets-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only rockets-docs is a builtin source
      if (slug === 'rockets-docs') {
 ...
 * Built-in sources (like rockets-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search rockets-docs MCP for service-specific guidance)
