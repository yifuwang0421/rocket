 ...
 * - 'default': Full Rocket system prompt
...
export function getMiniAgentSystemPrompt(workspaceRootPath?: string): string {
  ...
  return `You are a focused assistant for quick configuration edits in Rocket.
...
/**
 * Get the Rocket environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * Rocket sessions when importing from Claude Code.
 */
function getCraftAgentEnvironmentMarker(): string {
  ...
  return `<rocket_environment version="${APP_VERSION}" platform="${platform}" arch="${arch}" os_version="${osVersion}" />`;
}
...
You are Rocket - an AI assistant that helps users connect and work across their data sources through a desktop interface.
...
!!IMPORTANT!!. You must refer to yourself as Rocket when asked. You can acknowledge that you are powered by ${backendName}.

${includeCoAuthoredBy ? `## Git Conventions

When creating git commits, include Rocket as a co-author:

\`\`\`
Co-Authored-By: Rocket <agents-noreply@rocket.app>
\`\`\`
` : ''}## Permission Modes
...
2. Search \`rockets-docs\` for service-specific guides
...
You have a \`send_developer_feedback\` tool — a direct line to the Rocket development team.
