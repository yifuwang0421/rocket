...
export function isDevRuntime(): boolean {
  ...
  return nodeEnv === 'development' || nodeEnv === 'dev' || getEnv('ROCKET_DEBUG') === '1';
}
...
export function isDeveloperFeedbackEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('ROCKET_FEATURE_DEVELOPER_FEEDBACK'));
 ...
 *
 * Defaults to disabled. Override with ROCKET_FEATURE_EMBEDDED_SERVER=1|0.
 */
export function isEmbeddedServerEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('ROCKET_FEATURE_EMBEDDED_SERVER'));
  if (override !== undefined) return override;
  return false;
}
...
export const FEATURE_FLAGS = {
   ...
   * Override with ROCKET_FEATURE_DEVELOPER_FEEDBACK=1|0.
   */
