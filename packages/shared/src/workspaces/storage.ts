 ...
 * Default location: ~/.rocket/workspaces/
 */
...
const CONFIG_DIR = join(homedir(), '.rocket');
...
/**
 * Get the default workspaces directory (~/.rocket/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.rocket/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.rocket/workspaces/
 */
 ...
 * Resolution order: ENV (ROCKET_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.ROCKET_LOCAL_MCP_ENABLED;
