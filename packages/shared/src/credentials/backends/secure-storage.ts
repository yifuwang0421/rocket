 ...
 *
 * Stores credentials in an encrypted file at ~/.rocket/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.rocket');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('rocket-v2') // Bumped version for new key derivation
      .digest();
