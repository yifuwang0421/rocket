...
import { getWorkspaceByNameOrId, getGitBashPath, setGitBashPath, clearGitBashPath } from '@rocket/shared/config'
...
import { isUsableGitBashPath, validateGitBashPath } from '@rocket/server-core/services'
...
function parseInternalRocketDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  const runtimeScheme = process.env.ROCKET_DEEPLINK_SCHEME
  if (!isRocketDeepLinkScheme(parsed.protocol, runtimeScheme ? [runtimeScheme] : [])) return null
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.releaseNotes.GET, async () => {
    const { getCombinedReleaseNotes } = require('@rocket/shared/release-notes') as typeof import('@rocket/shared/release-notes')
    return getCombinedReleaseNotes()
  })

  server.handle(RPC_CHANNELS.releaseNotes.GET_LATEST_VERSION, async () => {
    const { getLatestReleaseVersion } = require('@rocket/shared/release-notes') as typeof import('@rocket/shared/release-notes')
  ...
  // Shell operations - open URL in external browser (or handle Rocket deep links internally)
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        const deepLink = parseInternalRocketDeepLink(parsed)
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing Rocket deep link internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for Rocket deep link')
