...
import { getWorkspaceByNameOrId, addWorkspace, setActiveWorkspace, updateWorkspaceRemoteServer } from '@rocket/shared/config'
...
export function registerWorkspaceCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.workspaces.CHECK_SLUG, async (_ctx, slug: string) => {
    const defaultWorkspacesDir = join(homedir(), '.rocket', 'workspaces')
  ...
  server.handle(RPC_CHANNELS.theme.GET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId: string) => {
    const { getWorkspaces } = await import('@rocket/shared/config/storage')
    const { getWorkspaceColorTheme } = await import('@rocket/shared/workspaces/storage')
  ...
  server.handle(RPC_CHANNELS.theme.SET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId: string, themeId: string | null) => {
    const { getWorkspaces } = await import('@rocket/shared/config/storage')
    const { setWorkspaceColorTheme } = await import('@rocket/shared/workspaces/storage')
    const workspaces = getWorkspaces()
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (!workspace) return
  ...
  server.handle(RPC_CHANNELS.views.LIST, async (_ctx, workspaceId: string) => {
    ...
    const { listViews } = await import('@rocket/shared/views/storage')
    return listViews(workspace.rootPath)
  })

  // Save views (replaces full array)
  server.handle(RPC_CHANNELS.views.SAVE, async (_ctx, workspaceId: string, views: import('@rocket/shared/views').ViewConfig[]) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const { saveViews } = await import('@rocket/shared/views/storage')
  ...
  server.handle(RPC_CHANNELS.toolIcons.GET_MAPPINGS, async () => {
    const { getToolIconsDir } = await import('@rocket/shared/config/storage')
    const { loadToolIconConfig } = await import('@rocket/shared/utils/cli-icon-resolver')
    const { encodeIconToDataUrl } = await import('@rocket/shared/utils/icon-encoder')
  ...
  server.handle(RPC_CHANNELS.logo.GET_URL, async (_ctx, serviceUrl: string, provider?: string) => {
    const { getLogoUrl } = await import('@rocket/shared/utils/logo')
