...
export interface SessionToolContext {
  ...
  /** Absolute path to workspace folder (~/.rocket/workspaces/{id}) */
   ...
   * - Claude: writes JSON files to ~/.rocket/feedback/
  ...
  /**
   * Create a Rocket Task (board card + task.yaml + orchestrator session)
...
export interface ResolvedLabelsResult {
   ...
   * Populated by `resolveSessionLabels()` from `@rocket/shared/labels`.
