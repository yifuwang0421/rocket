...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to ROCKET_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.ROCKET_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.ROCKET_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.ROCKET_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute ROCKET_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute ROCKET_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (ROCKET_UV / ROCKET_NODE / ROCKET_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.ROCKET_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ROCKET_UV, 'Python/uv')
        : process.env.ROCKET_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure ROCKET_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.ROCKET_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ROCKET_NODE, 'Node')
        : process.env.ROCKET_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure ROCKET_NODE or install node on PATH.'
    );
  }

  if (process.env.ROCKET_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.ROCKET_BUN, 'Bun')
      : process.env.ROCKET_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure ROCKET_BUN or install bun on PATH.'
  );
}
