 ...
 * It uses the shared handlers from @rocket/session-tools-core to ensure
 ...
 *   --workspace-root: Path to workspace folder (~/.rocket/workspaces/{id})
...
import { isDeveloperFeedbackEnabled } from '@rocket/shared/feature-flags';
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.rocket/workspaces/{id}
      // preferencesPath = ~/.rocket/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.ROCKET_CONFIG_DIR || join(workspaceRootPath, '..', '..');
...
// ============================================================
// Rocket Docs Upstream Proxy
// ============================================================

const DOCS_MCP_URL = 'https://agents.rocket.app/docs/mcp';
...
/**
 * Connect to the rockets-docs MCP server and fetch its tool definitions.
...
async function connectDocsUpstream(): Promise<void> {
  try {
    const client = new Client(
      { name: 'rocket-session-proxy', version: '1.0.0' },
      { capabilities: {} }
    );
    ...
    console.error(`Rocket Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`Rocket Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`Rocket Docs server is not connected. Tool '${name}' unavailable.`);
  }
...
async function handleCallLlm(
  ...
  // Uses callbackPort from CLI arg (--callback-port) or env var (ROCKET_LLM_CALLBACK_PORT).
  ...
  return errorResponse(
    ...
    'HTTP callback (ROCKET_LLM_CALLBACK_PORT). Neither is available.'
  );
}
...
async function main() {
  ...
  const config: SessionConfig = {
    ...
    callbackPort: callbackPort || process.env.ROCKET_LLM_CALLBACK_PORT,
  };
