/**
 * Server spawner — start a headless Rocket server as a child process.
 *
 * Spawns `bun run <serverEntry>`, reads stdout for the `ROCKET_SERVER_URL=`
 * and `ROCKET_SERVER_TOKEN=` lines, and returns a handle to stop the server.
 */
...
export async function spawnServer(opts?: SpawnServerOptions): Promise<SpawnedServer> {
  ...
  // Read stdout line by line looking for ROCKET_SERVER_URL=
  return new Promise<SpawnedServer>((resolve, reject) => {
    ...
    const processLines = () => {
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('ROCKET_SERVER_TOKEN=')) {
    ...
    ;(async () => {
      ...
      clearTimeout(timer)
      if (!url) {
        reject(new Error('Server process exited before printing ROCKET_SERVER_URL'))
      }
    })()
  })
}
