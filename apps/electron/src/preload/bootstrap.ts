 ...
 *
 * Thin-client mode (ROCKET_SERVER_URL):
...
import { createCallbackServer } from '@rocket/shared/auth/callback-server'
import { CHATGPT_OAUTH_CONFIG } from '@rocket/shared/auth/chatgpt-oauth-config'
import {
...
import type { ConfirmDialogSpec, FileDialogSpec, BrowserCapabilityRequest } from '@rocket/server-core/transport'
import type { RpcClient } from '@rocket/server-core/transport'
import type { RemoteServerConfig } from '@rocket/core/types'
...
if (isClientOnly) {
  ...
  const wsUrl = process.env.ROCKET_SERVER_URL!
  const wsToken = process.env.ROCKET_SERVER_TOKEN ?? ''
  ...
  if (parsed.protocol === 'ws:' && !isLocalhost) {
    throw new Error(
      ...
      `Set ROCKET_RPC_TLS_CERT/KEY on the server to enable TLS.`
    )
  }

  // Workspace ID is optional — if missing, renderer shows a workspace picker
  const workspaceId = process.env.ROCKET_WORKSPACE_ID || ipcRenderer.sendSync('__get-workspace-id') || undefined
...
;(api as ElectronAPI).getSystemWarnings = async () => ({
  vcredistMissing: process.env.ROCKET_VCREDIST_MISSING === '1',
  downloadUrl: process.env.ROCKET_VCREDIST_URL,
})
