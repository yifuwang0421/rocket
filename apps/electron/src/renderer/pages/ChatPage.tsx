...
const ChatPage = React.memo(function ChatPage({ sessionId, overlayPresentation = 'auto', researchContexts, onRemoveResearchContext, beforeResearchContextSubmit }: ChatPageProps) {
  ...
  const handleOpenInNewWindow = React.useCallback(async () => {
    ...
    const url = `rocket://${route}${separator}window=focused`
    try {
      ...
      <StyledDropdownMenuContent align="end" sideOffset={8}>
        {sharedUrl ? (
          <>
            ...
            <StyledDropdownMenuSeparator />
            <StyledDropdownMenuItem onClick={() => window.electronAPI.openUrl('https://agents.rocket.app/docs/go-further/sharing')}>
