 ...
 * Deep Link Handler
 *
 * Parses canonical rocket:// URLs and the legacy craftagents:// compatibility
 * alias, then routes them to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   rocket://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   rocket://flagged[/session/{sessionId}]                - Session list (flagged filter)
 *   rocket://state/{stateId}[/session/{sessionId}]        - Session list (state filter)
 *   rocket://sources[/source/{sourceSlug}]                - Sources list
 *   rocket://settings[/{subpage}]                         - Settings
 *
 * Action format:
 *   rocket://action/{actionName}[/{id}][?params]
 *   rocket://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 * Examples:
 *   rocket://allSessions
 *   rocket://allSessions/session/abc123
 *   rocket://settings/shortcuts
 *   rocket://sources/source/github
 *   rocket://action/new-chat
 *   rocket://action/resume-sdk-session/{sdkId}
 *   rocket://workspace/ws123/allSessions/session/abc123
 */
...
import { isRocketDeepLinkScheme } from '@rocket/shared/branding'
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    const runtimeScheme = process.env.ROCKET_DEEPLINK_SCHEME
    if (!isRocketDeepLinkScheme(parsed.protocol, runtimeScheme ? [runtimeScheme] : [])) {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., rocket://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., rocket://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // rocket://auth-callback?... (OAuth callbacks are handled elsewhere)
    ...
    // rocket://allSessions/..., rocket://settings/..., etc. (compound routes)
    ...
    // rocket://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // rocket://action/... (no workspace - uses active window)
    if (host === 'action') {
