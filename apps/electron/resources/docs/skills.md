...
This guide explains how to create and configure skills in Rocket.

> The P0 Rocket CLI is a server client; it does not manage skills. Use the
> in-app skill flow or edit `SKILL.md` using the format described below.
...
Rocket uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in Rocket
...
1. **Workspace skill checked first** - If `~/.rocket/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.rocket/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.rocket/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.rocket/workspaces/{ws}/skills/commit/SKILL.md`
