...
This guide explains how to configure automations in Rocket to automate workflows based on events.

> The P0 Rocket CLI is a server client; it does not manage automations. Use the
> in-app automation flow or edit and validate `automations.json`.

## What Are Automations?

Automations allow you to trigger actions automatically when specific events occur in Rocket. You can:
...
```
~/.rocket/workspaces/{workspaceId}/automations.json
```
...
### App Events (triggered by Rocket)
...
Send a prompt to Rocket (creates a new session for scheduled prompts).
...
- Environment variables are expanded (e.g., `$ROCKET_LABEL`)
...
{
  "type": "webhook",
  "url": "https://hooks.slack.com/services/${ROCKET_WH_SLACK_PATH}",
  "method": "POST",
  "body": {
    "text": "Session ${ROCKET_SESSION_NAME} status changed to ${ROCKET_NEW_STATE}"
  }
}
```
...
```json
{
  ...
  "auth": {
    "type": "bearer",
    "token": "${ROCKET_WH_API_TOKEN}"
  },
  "body": { "event": "$ROCKET_EVENT" }
}
```
...
```json
{
  ...
  "auth": {
    "type": "basic",
    "username": "${ROCKET_WH_USER}",
    "password": "${ROCKET_WH_PASS}"
  }
}
```
...
**Security:** Webhook actions only have access to `ROCKET_*` system variables and `ROCKET_WH_*` user-defined secrets. They do **not** have access to your full system environment (e.g., `$HOME`, `$PATH`, or other process variables).

## Environment Variables

Both prompt and webhook actions support variable expansion using `$VAR` or `${VAR}` syntax.

### System Variables (ROCKET_*)

These are automatically set by the automation system based on the triggering event:

| Variable | Description | Available For |
|----------|-------------|---------------|
| `$ROCKET_EVENT` | Event name (e.g., `LabelAdd`) | All events |
| `$ROCKET_EVENT_DATA` | Full event payload as JSON | All events |
| `$ROCKET_SESSION_ID` | Session ID | Events with session context |
| `$ROCKET_SESSION_NAME` | Session name | Events with session context |
| `$ROCKET_WORKSPACE_ID` | Workspace ID | All events |

**Per-event variables:**

| Event | Variable | Description |
|-------|----------|-------------|
| `LabelAdd` / `LabelRemove` | `$ROCKET_LABEL` | The label that was added/removed |
| `PermissionModeChange` | `$ROCKET_OLD_MODE`, `$ROCKET_NEW_MODE` | Previous and new permission mode |
| `FlagChange` | `$ROCKET_IS_FLAGGED` | `true` or `false` |
| `SessionStatusChange` | `$ROCKET_OLD_STATE`, `$ROCKET_NEW_STATE` | Previous and new status |
| `SchedulerTick` | `$ROCKET_LOCAL_TIME`, `$ROCKET_LOCAL_DATE` | Current time (`14:30`) and date (`2026-03-09`) |

### User-Defined Webhook Secrets (ROCKET_WH_*)

For webhook actions, you can define your own secrets by setting environment variables with the `ROCKET_WH_` prefix in your shell profile (e.g., `~/.zshrc`, `~/.bashrc`):

```bash
# In your shell profile
export ROCKET_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
export ROCKET_WH_DISCORD_URL="https://discord.com/api/webhooks/123/abc"
export ROCKET_WH_API_TOKEN="your-secret-token"
```
...
{
  "type": "webhook",
  "url": "${ROCKET_WH_SLACK_URL}",
  "method": "POST",
  "body": { "text": "Hello from Rocket!" }
}
```
...
```json
{
  "type": "webhook",
  "url": "https://api.example.com/events",
  "headers": { "Authorization": "Bearer ${ROCKET_WH_API_TOKEN}" },
  "body": { "event": "${ROCKET_EVENT}", "session": "${ROCKET_SESSION_NAME}" }
}
```

This keeps secrets out of `automations.json` (which may be shared or committed to version control).

> **Note:** Only variables prefixed with `ROCKET_WH_` are injected into webhook actions. Other environment variables (like `$HOME` or `$DATABASE_URL`) are not accessible to webhooks.
...
4. **Pair the supergroup.** In Rocket: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.

Verify by checking the supergroup row in Settings shows the group title. If automation runs fail later, `~/.rocket/logs/messaging-gateway.log` will show `automation_topic_bind_failed` with the underlying Telegram error.
        ...
        "actions": [
          {
            ...
            "url": "${ROCKET_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":warning: Permission escalated from safe to allow-all in *${ROCKET_SESSION_NAME}*" }
          }
        ]
      }
    ]
...
```json
{
  ...
  "automations": {
    "LabelAdd": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $ROCKET_LABEL was added. Log this change with a timestamp." }
        ]
      }
    ],
    "LabelRemove": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $ROCKET_LABEL was removed. Log this change with a timestamp." }
        ]
      }
    ]
  }
...
Sends a Slack message when a session is marked as done. Requires `ROCKET_WH_SLACK_URL` in your shell profile.
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "body": {
              "text": ":white_check_mark: Session *${ROCKET_SESSION_NAME}* marked as done"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "url": "${ROCKET_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":rotating_light: Urgent label added to *${ROCKET_SESSION_NAME}*" }
          },
          {
            "type": "prompt",
        ...
        "actions": [
          {
            ...
            "body": {
              ...
              "client_id": "${ROCKET_WH_CLIENT_ID}",
              "client_secret": "${ROCKET_WH_CLIENT_SECRET}"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "headers": {
              "Authorization": "Bearer ${ROCKET_WH_API_TOKEN}",
              "X-Source": "rocket"
            },
            "body": {
              "event": "${ROCKET_EVENT}",
              "session_id": "${ROCKET_SESSION_ID}",
              "old_status": "${ROCKET_OLD_STATE}",
              "new_status": "${ROCKET_NEW_STATE}"
            }
          }
        ]
      }
...
Ask Rocket to validate your automations configuration:
...
2. **Check env vars** — Ensure `ROCKET_WH_*` variables are set in your shell profile and Rocket was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
...
5. **Keep secrets out of config** - Use `ROCKET_WH_*` env vars for webhook URLs and tokens instead of hardcoding them in automations.json
