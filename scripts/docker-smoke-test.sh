#!/usr/bin/env bash
# =============================================================================
# Docker smoke test for Rocket Server
#
...
docker run -d \
  --name "$CONTAINER_NAME" \
  -p "$PORT:9100" \
  -e "ROCKET_SERVER_TOKEN=$TOKEN" \
  -e "ROCKET_RPC_HOST=0.0.0.0" \
  -e "ROCKET_RPC_PORT=9100" \
  "$IMAGE"
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
  ...
  if docker logs "$CONTAINER_NAME" 2>&1 | grep -q "ROCKET_SERVER_URL="; then
    READY=true
    break
  fi
