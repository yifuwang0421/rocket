#!/usr/bin/env bun
/**
 * Build script for standalone Rocket server.
 *
...
function showHelp(): void {
  console.log(`
Standalone server build script for Rocket
 ...
 * not node: builtins, not workspace @rocket/* packages).
 */
    ...
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== 'tests' && entry.name !== '__tests__') {
        ...
        while ((match = importRe.exec(content)) !== null) {
          ...
          if (spec.startsWith('.') || spec.startsWith('node:') || spec.startsWith('@rocket/')) continue;
...
function copyWorkspacePackages(config: ServerBuildConfig): void {
  ...
  // ROCKET_MESSAGING_WA_WORKER env resolution in packages/server/src/index.ts.
  const packages = [
    'server',
    'server-core',
    'shared',
...
function createRootConfig(config: ServerBuildConfig): void {
  ...
  // Root package.json with workspaces (Bun resolves @rocket/* through this)
  const rootPkg = {
    name: 'rocket-server-dist',
    version,
    private: true,
    workspaces: ['packages/*'],
  };
  ...
  const rootTsconfig = {
    compilerOptions: {
      ...
      paths: {
        '@rocket/server-core/*': ['./packages/server-core/src/*'],
        '@rocket/shared/*': ['./packages/shared/src/*'],
        '@rocket/core/*': ['./packages/core/src/*'],
        '@rocket/session-tools-core/*': ['./packages/session-tools-core/src/*'],
      },
    },
  };
  writeFileSync(join(outputDir, 'tsconfig.json'), JSON.stringify(rootTsconfig, null, 2) + '\n');

  // Create workspace symlinks in node_modules/@rocket/
  // Bun needs these to resolve workspace package imports at runtime
  const scopeDir = join(outputDir, 'node_modules', '@rocket');
    ...
    for (const pkg of readdirSync(packagesDir)) {
      ...
      try {
        ...
        if (name.startsWith('@rocket/')) {
          const shortName = name.replace('@rocket/', '');
...
function createEntryScripts(config: ServerBuildConfig): void {
  ...
  // bin/rocket-server — main entry wrapper
  const rocketServer = `#!/bin/sh
set -e
...
export ROCKET_BUNDLED_ASSETS_ROOT="$ROOT"
export ROCKET_IS_PACKAGED=true
export ROCKET_APP_ROOT="$ROOT"
export ROCKET_RESOURCES_PATH="$ROOT/resources"

# CLI tools (doc tools use uv + Python scripts)
export ROCKET_UV="$ROOT/resources/bin/uv"
export ROCKET_SCRIPTS="$ROOT/resources/scripts"
...
`;
  writeFileSync(join(binDir, 'rocket-server'), rocketServer);

  // start.sh — convenience entry
  const startSh = `#!/bin/sh
# Rocket Server — convenience entry point
DIR="$(cd "$(dirname "$0")" && pwd)"
exec "$DIR/bin/rocket-server" "$@"
`;
...
echo "=== Rocket Server Setup ==="
echo ""

# Make binaries executable
chmod +x "$DIR/bin/rocket-server" "$DIR/start.sh"
...
if [ -z "\${ROCKET_SERVER_TOKEN:-}" ]; then
  TOKEN=\$(openssl rand -hex 32)
  cat > "$DIR/.env" <<ENVFILE
ROCKET_SERVER_TOKEN=$TOKEN

# TLS — uncomment and set paths to enable wss://
# ROCKET_RPC_TLS_CERT=/path/to/cert.pem
# ROCKET_RPC_TLS_KEY=/path/to/key.pem
# ROCKET_RPC_TLS_CA=/path/to/ca.pem
ENVFILE
  echo ""
  echo "Generated server token (saved to $DIR/.env)"
else
  TOKEN="\$ROCKET_SERVER_TOKEN"
  echo ""
  echo "Using ROCKET_SERVER_TOKEN from environment."
fi
...
if [ "\${1:-}" = "--systemd" ]; then
  ...
  SERVICE_USER="\${ROCKET_USER:-\$(logname 2>/dev/null || echo rocket)}"
  SERVICE_FILE="/etc/systemd/system/rocket-server.service"
...
Environment=ROCKET_RPC_HOST=127.0.0.1
Environment=ROCKET_RPC_PORT=9100
ExecStart=$DIR/bin/rocket-server
Restart=on-failure
RestartSec=5
...
UNIT
  ...
  echo "  Start:   sudo systemctl start rocket-server"
  echo "  Status:  sudo systemctl status rocket-server"
  echo "  Logs:    journalctl -u rocket-server -f"
  echo ""
  exit 0
fi
...
echo ""
echo "Quick start:"
echo "  ROCKET_SERVER_TOKEN=$TOKEN $DIR/start.sh"
echo ""
...
# Make binaries executable
RUN chmod +x bin/rocket-server vendor/bun/bun resources/bin/uv && \\
    for f in resources/bin/*; do [ -f "$f" ] && chmod +x "$f"; done

ENV ROCKET_IS_PACKAGED=true
ENV ROCKET_BUNDLED_ASSETS_ROOT=/app
ENV ROCKET_APP_ROOT=/app
ENV ROCKET_RESOURCES_PATH=/app/resources
ENV ROCKET_UV=/app/resources/bin/uv
ENV ROCKET_SCRIPTS=/app/resources/scripts
ENV ROCKET_RPC_HOST=0.0.0.0
ENV ROCKET_RPC_PORT=9100
ENV PATH="/app/resources/bin:/app/vendor/bun:\${PATH}"

EXPOSE 9100

ENTRYPOINT ["/app/bin/rocket-server"]
`;
...
services:
  rocket-server:
    ...
    ports:
      - "9100:9100"
    environment:
      - ROCKET_SERVER_TOKEN=\${ROCKET_SERVER_TOKEN:?Set ROCKET_SERVER_TOKEN}
      - ROCKET_RPC_PORT=9100
      # TLS — uncomment to enable wss://
      # - ROCKET_RPC_TLS_CERT=/certs/cert.pem
      # - ROCKET_RPC_TLS_KEY=/certs/key.pem
    volumes:
      - rocket-data:/root/.rocket
...
async function main(): Promise<void> {
  ...
  console.log(`=== Building Rocket Server ${version} for ${platform}-${arch} ===`);
  ...
  if (config.compress) {
    const archiveName = `rocket-server-${version}-${platform}-${arch}.tar.gz`;
  ...
  console.log(`\nQuick start:`);
  console.log(`  ROCKET_SERVER_TOKEN=<secret> ${outputDir}/start.sh`);
}
